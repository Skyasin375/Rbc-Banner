...
# Your Info API - shappno-vip-info
INFO_API_URL = "http://info-ob55-shappnooo.vercel.app/shappno"
...
@app.get("/")
async def home():
    return {"status": "Banner API Running", "endpoint": "/shappno-banner?uid=UID"}


@app.get("/shappno-banner")
async def get_banner(uid: str):
    if not uid:
        raise HTTPException(status_code=400, detail="UID required")

    # =============================================
    # Info API থেকে সব ডেটা নেওয়া - shappno-vip-info format
    # =============================================
    data = await fetch_info(uid)
    
    # ডেটা পার্স করা - shappno-vip-info direct response
    payload = data
    
    # basicInfo থেকে ডেটা নেওয়া
    basic = payload.get("basicInfo", {}) if isinstance(payload, dict) else {}
    
    if not basic:
        raise HTTPException(status_code=404, detail="Account not found")

    # clanBasicInfo থেকে ডেটা নেওয়া
    clan = payload.get("clanBasicInfo", {}) if isinstance(payload, dict) else {}
    
    # সব ডেটা এক্সট্রাক্ট
    name = basic.get("nickname", basic.get("Name", "Unknown"))
    level = basic.get("level", basic.get("Level", "0"))
    guild_name = clan.get("clanName", clan.get("GuildName", ""))
    
    # ID গুলো - shappno-vip-info format
    avatar_id = basic.get("headPic", basic.get("AvatarId", basic.get("avatarId", 0)))
    banner_id = basic.get("bannerId", basic.get("BannerId", basic.get("banner", 0)))
    pin_id = basic.get("pinId", basic.get("PinId", 0))
